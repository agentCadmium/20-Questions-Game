...
/**
 * Assignment4
 * @author AnastasiaSaverchenko
 * Received assistance from Yavneeka at drop-in hours
 * 
 */
...
public class LinkedList<T> {
	 ...
	 * @param the current node, after which the new node is inserted, the stored in the new node
	 */
	public void insertAfter( LinkedListNode<T> currentNode, T data ){
		...
		if(head!=null){
			...
			//se the new node as the next for current node
	...
	public void deleteFirst(){
		if(head!=null){
		//move head pointed to the node after the first node	
		head = head.getNext();
		}
		else{
			System.out.println("List is empty");
	...
	public void deleteLast(){
		...
		if(head.getNext() == null) {
			...
			//remove the link connecting the previous to last node and the last node
			//removes last node
